...
\beqn
	...
	\frac{1}{4\pi}\, \sum_{i=1}^{N-1} \Bigl( iD ~+~ 2\,\Bigl|\sigma - \frac{m_i}{\sqrt{2}}\Bigr|^2 \Bigr)\,
		\ln\, \frac{ iD \,+\, 2 \left| \sigma - \frac{\displaystyle m_i}{\displaystyle \sqrt{2}} \right|^2} {\Lambda^2}
\nonumber\\[4mm]
	&+& 
	\frac{1}{4\pi}\, \sum_{i=1}^{N-1} 2\,\Bigl| \sigma - \frac{m_i}{\sqrt{2}}\Bigr|^2\,
			\ln\, \frac{ 2\, \left| \sigma - \frac{ m_i}{\sqrt{2}} \right|^2 }{ \Lambda^2 }
	+
	\frac{1}{4\pi}\, iD\, (N-1)  \,  .
	\nonumber\\
	\label{Veff22}
...
Now, we switch on the heterotic deformation breaking \ntt\, supersymmetry down to \nzt.
...
heterotic
model, just as in Sect.~\ref{lnscptm}, we integrate over all but one given $ n^l $ filed (and its superpartner $ \xi^l $).
...
\nonumber\\[4mm]
	...
	\frac{1}{4\pi}\, \sum_{i=1}^{N-1} 2\,\Bigl|\sigma - \frac{m^i}{\sqrt{2}}\Bigr|^2\,
			\ln\, \frac{ 2\, \left| \sigma - \frac{\displaystyle m^i}{\displaystyle \sqrt{2}} \right|^2 } { \Lambda^2 }
	+
	\frac{1}{4\pi}\, iD\, (N-1) 
+
	4\, |\omega|^2\,\bigl|\sigma\bigr|^2 \,.
	\nonumber\\
	\label{Veff}
\eeqn
It is identical to Eq.~(\ref{Veff22}) except the last term representing the heterotic deformation.
Now, to find the vacua, we must minimize the 
...
will solve these equations perturbatively, expanding in powers of  $ |\omega|^2 $,
\beqn
	n &=& \nz  + |\omega|^2\,\no + \ldots\,,
	\nonumber
	 \\[3mm]
...
&&
	...
	\label{higgseq}
\eeqn
	We see that $ iD $ vanishes to the first order in $ |\omega|^2 $.
	Thus, for $ iD $ the first-order expansion is insufficient, since we know that supersymmetry
	is broken (see \cite{SYhet,BSY3}) and, hence, $ iD  \neq  0 $.
	However, it is easy to recover $ iD $ 
	 in the Higgs phase to order $O(|\omega |^4)$ given the corresponding expansion of $ \sigma $
	via Eq.~(\ref{higgsph}).
...
\beqn
	...
	\sigz 
	&=&
	 \frac{m_0}{\sqrt{2}}\,,  \qquad       \sigo  = -\,\frac{2\sigz}{|\nz|^2}\,, 
	 \nonumber\\[3mm]
	|\nz|^2 
	&=&
	2\beta_{\rm ren}\,,              \qquad   
			\no =  -\,\frac{2\,m^0}{\bnz\,|\nz|^2}\,\frac{1}{4\pi}\,\sum_{i=1}^{N-1} \frac{1}{m^0 - m^i}\,.
			\label{higgseqp}
			\eeqn
...
\beq
	...
	\label{higgseqpp}
\eeq
Assembling all these expressions we arrive at
\beqn
	\sigma 
	&=&
	 \frac{m^0}{\sqrt{2}}\, \lgr 1 - \frac{2\,|\omega|^2}{|\nz|^2} \rgr + \ldots \,,
	\nonumber\\[2mm]
	i D  
	&=&
	...
	&=&
	 ...
	 \label{finhi}
\eeqn
This is in the  the Higgs phase, where
$$
...
Our starting point is the zeroth order in $ |\omega|^2 $ solution (Sect.~\ref{lnscptm}),
\beq
	\nz = 0\,, \quad
	i \Dz = 0\,, \quad
	...
	where
\beq
	\wt{\Lambda} = \sqrt[N]{ \Lambda^N + m^N } =\Lambda\left[1 +O\left(e^{-N}\right)
	\right]\,\,\, {\rm at}\,\,\, N\to\infty\,.
	\label{tilla}
\eeq
	At strong coupling $n$ vanishes exactly, not only in the zeroth order in $\omega$.
	The remaining two equations (\ref{eff1}) and (\ref{eff3}) imply, to the first order in $ |\omega|^2 $
\beqn
&&
	\sum_{i=1}^{N-1}\,\, \frac{ i \Do + 2 \lgr \bsigz\,\sigo -
	...
	\nonumber\\[4mm]
&&
	\frac{1}{4\pi}\,\, \sum_{i=1}^{N-1}\, \frac{i \Do}
				    {2\,\Bigl( \bsigz - \frac{\displaystyle\ov{m}{}^i}{\displaystyle \sqrt{2}} \Bigr) }
			= 2\, \sigz\,.
\eeqn
The solution to these equations is given by (in conjunction with $n=0$)
\beqn
&&
 \Dz = 0\,,\qquad
	...
	\nonumber\\[3mm]
&&
	\sigo \, \sum_{i=1}^{N-1} \frac{1}
			{ \sigz - \frac{ m^i}{\sqrt{2}} }
		+ \text{H.c.} =
	-\, 8\pi  \sigz  \,\,
	\frac{ \sum_{i=1}^{N-1} 
			\frac{1}{ \large| \sigz - \frac{ m^i}{\sqrt{2}} \large|^2 } }
	  {  \sum_{i=1}^{N-1} 
				\frac{1}{ \bsigz - \frac{\ov{m}{}^i}
							{\sqrt{2}} } }\,.
\label{strongeq}
\eeqn
We use the following relations to simplify the above formule for masses (\ref{two}):
\beqn
	&&
...
&&
	...
	+
	\frac{2\,N}{ {\Lambda}{}^2 - m^2 } \,,
%	\cdot
%	\frac{ \wt{\Lambda}{}^N + m^N } { \Lambda^N }\,.
	\label{vspforp}
\eeqn
where only  the leading-$N$ contributions  are retained.
Substituting (\ref{vspforp})    in  Eq.~(\ref{strongeq}) we get
\beqn
&&
	i \Do  = \frac{16\pi}{N}\,  |\sigz|^2\,,
	\nonumber\\[3mm]
&&
	| \sigo | =
	-\, \frac{8\pi}{N}\, 
		\frac{ {\Lambda}{}^3 } {{\Lambda}{}^2 - m^2 }\,
		%\Bigl[ 1 + \frac{m^N}{\wt{\Lambda}{}^N} \Bigr] \cdot
		\left[
		 \cos \left( \frac{2\pi l }{N} - \varphi \right) \right]^{-1}\,,
		\label{vspforpp}
\eeqn
	where $ \varphi $ is the arbitrary phase of $ \sigo $.
	We must choose this phase to be the same as that of $ \sigz $, namely, $\varphi \equiv {2\pi l}/{N}$; then  
 the cosine disappears. Introducing a dimensionless deformation parameter 
 \beq
u\equiv \frac{8\pi}{N}\,|\omega|^2
\label{u}
\eeq	
	we can present our $m\ll\Lambda$ results in the following quite simple form:
\beqn
	n 
	&=&
	 0 \,,\qquad i D   =  u\, \Lambda^2 + \ldots \,, 
	\nonumber\\[3mm]
	\sqrt{2}\sigma & =& \Lambda \, e^{\frac{2\pi i l}{N}} \left[1
			~-~ \frac{u}{2}\, 
...
that interpolate between, say, thetrue vacuum and the lowest quasivacuum \cite{GSY05},
...
\beqn
&&
{\rm ln}\left( {i D + 2\, \Bigl| \sigma -  \frac{\displaystyle m^i}{\displaystyle \sqrt{2}} \Bigr|^2}\right)
=
