...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
\vspace{0.7cm}
We now will take a close look at what happens with $\sigma $ at the Strong/Coloumb 
...
just by inspecting Eq.~\eqref{scpheq3}. 
To solve this equation we can evaluate the sum in it using Eq.~\eqref{usefulformula}.
In place of the massive quantities $ \sigma $, $\Lambda $ and $ m $ it is convenient to 
temporarily introduce dimensionless variables
\beq
	\cs ~~=~~ \frac{\sqrt{2} \sigma}{\Lambda}\,, \qquad\qquad
        \mu ~~=~~ m/\Lambda\,.
\label{csdef}
\eeq
Then equation \eqref{scpheq3} turns into
\beq
\label{mu-s}
	\mu^2  ~~=~~ - \cs^2 \lgr u  ~+~ \ln \cs^2 \rgr .
\eeq
Instead of solving for $ \cs $ one can use \eqref{mu-s} as a solution for $ m $ with respect 
to $ \sigma $.
%\begin{figure}
%\epsfxsize=10cm
...
\caption{Dependence of $m^2$ versus $\sigma^2$. The rescaled variables as in text are
$\mu = m/\Lambda$ and $\cs = \sqrt{2}\sigma/\Lambda$. }
\label{fig:mus}
\end{figure}
Fig.~\ref{fig:mus} illustrates the dependence \eqref{mu-s}. 
...
This happens at $ \mu^2 = \cs^2 = e^{-(1+u)}$, as can be seen from Eq.~\eqref{mu-s}. 
...
\eeq
Equations \eqref{mu-s}, \eqref{scirc} together with Fig.~\ref{fig:mus} are approximate, but qualitatively
they very closely show what happens to $ \sigma $ in reality. 
Namely, $ \sigma $ monotonically decreases with increase of mass, 
until it experiences a vertical bend, at which point
it drops down to zero, the fact that cannot be seen from Eq.~\eqref{mu-s}. 
...
(see Fig.~\ref{fig:numEm}), and so the phase transition is at most second order. 
...
\caption{The dependence of $\sigma$ on $m$. The solid curve shows the numerical solution
...
\label{fig:numsigm}
\end{figure}
To see what happens at the Strong to Coulomb Phase Transition we solve vacuum equations 
\eqref{eff1}-\eqref{eff3} numerically, with $ n ~=~ 0 $.
Figure~\ref{fig:numsigm} shows the dependence of $ \sigma $ on $ m $ in the strong
coupling phase. 
The main graph is compared to the small-$m$ solution \eqref{smstr} and the large-$u$
dependence \eqref{mu-s} (still, resolved numerically). 
One convinces that the large-$u$ solution given by Eqs.~\eqref{scpheq3} and \eqref{mu-s}
...
energy density Eq.~\eqref{confEvac}.
\begin{figure}
\epsfxsize=10cm
\centerline{\epsfbox{publish_E.eps}}
\caption{Energy density versus $m$ in the strong coupling and Coulomb phase for $u=2$. 
...
\caption{Dependence of $\sigma$ and coupling constant $|n|^2$ on $m$ in the Higgs phase. 
...
Strictly speaking, Higgs phase vacuum equations become invalid as soon as $|n|^2$ gets to zero,
...
\beq
	\mu_* ~~=~~ \frac{1}{\cs_*}\,.
\eeq
This first equation has two solutions. 
...
\beq
{\mathcal S} = \gamma_\ssm\,{\mathcal W}_{\rm mirror}\,.
  \label{arithree}
 \eeq
 ...
 The scalar potential of the \nzt mirror Wess--Zumino model 
  is \cite{SYneww}
 \beq
 V= |\cw^{\,\prime} |^2+|\cs|^2 = |\cw^{\,\prime}_{\rm mirror} |^2 +|\gamma_\ssm |^2\,|\cw_{\rm mirror} |^2\,.
   \label{arifour}
 \eeq
...
Two-dimensional right- and left-handed derivatives are defined to be
\[
...
The first two terms in Eq.~\eqref{appabiferm} originally were introduced in Ref.~\cite{SY1}, with 
a constant
\beq
